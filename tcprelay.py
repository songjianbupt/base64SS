...
class TCPRelayHandler(object):
    ...
    def _handle_stage_connecting(self, data):
        if self._is_local:
            if self._encryptor is not None:
                logging.debug("before send data %s" % (binascii.hexlify(data)))
                data = self._protocol.client_pre_encrypt(data)
                data = self._encryptor.encrypt(data)
                #add by songjian
                #data = self._encryptor.encrypt(data)
                data = self._obfs.client_encode(data)
                #logging.warn("send data %s" % (binascii.hexlify(data)))
        if data:
            self._data_to_write_to_remote.append(data)
    ...
    def _handle_stage_addr(self, ogn_data, data):
        try:
            ...
            if self._is_local:
                ...
                if self._encryptor is not None:
                    data = self._protocol.client_pre_encrypt(data)
                    #logging.warn("addr send data %s %s" % (binascii.hexlify(data), self._remote_udp))
                    data_to_send = self._encryptor.encrypt(data)
                    #logging.warn("middle addr send %s" % (binascii.hexlify(data_to_send)))
                    #add by songjian
                    #data_to_send = self._encryptor.encrypt(data_to_send)
                    data_to_send = self._obfs.client_encode(data_to_send)
                    #logging.warn("after addr send data %s" % (binascii.hexlify(data_to_send)))
                if data_to_send:
            ...
            if self._encryptor is not None:
                if self._encrypt_correct:
                    ...
                    if obfs_decode[1]:
                        ...
                        logging.debug("recv data %s" % (binascii.hexlify(obfs_decode[0])))
                        data = self._encryptor.decrypt(obfs_decode[0])
                        logging.debug("recv decrypt data %s" % (binascii.hexlify(obfs_decode[0])))
                        #add by songjian
    ...
    def _on_remote_write(self):
        ...
        if self._data_to_write_to_remote:
            data = b''.join(self._data_to_write_to_remote)
            logging.debug("remote write %s" % (binascii.hexlify(data)))
            self._data_to_write_to_remote = []
            self._write_to_sock(data, self._remote_sock)
        else:
            self._update_stream(STREAM_UP, WAIT_STATUS_READING)
